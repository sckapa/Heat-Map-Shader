Shader "Unlit/HeatmapShader"
{
  Properties
  {
    ...
    _Color0("Color 0",Color) = (0,0,0,1)
      ...
      _DecayRate ("Decay Rate", Range(0, 1)) = 0.1
  }
    SubShader
    {
      ...
      Pass
      {
        ...
        //If distance > 1.0, zero contribution
        float distsq(float2 a, float2 b, float timeFactor)
        {
          float area_of_effect_size = _Diameter * timeFactor;
        ...
        fixed4 frag(v2f i) : SV_Target
        {
          ...
          float timeFactor = 1.0 - _Time.y * _DecayRate; //Decay rate property to control the speed of decay
          for (float i = 0.0; i < _HitCount; i++)
          {
            float2 work_pt = float2(_Hits[i * 3], _Hits[i * 3 + 1]);
            float pt_intensity = _Hits[i * 3 + 2] * timeFactor;

            totalWeight += 0.5 * distsq(uv, work_pt, timeFactor) * pt_intensity * _Strength * (1 + sin(_Time.y * _PulseSpeed));
          }
